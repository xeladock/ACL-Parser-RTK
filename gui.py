...
Чекбоксы:
  ...
  • Фильтр по оборудованию - позволяет выбрать конкретный тип устройств для поиска ACL.
...
Сохранение:
  • Меню "Программа" - "Сохранить на диск" — открывает диалоговое окно для сохранения вывода данных.
        Если поле вывода пустое будет выведено предупреждение.

Удаление:
  • Меню "Программа" - "Удалить папку конфигураций" —  удаляет папку с конфигурациями "config_files_clear".
        После удаления программа вернется в состояние ввода логина/пароля gitlab/netbox Token для повторной загрузки.
...
Утилита "OG Viewer":
  Утилита распаковывает object-group устройств поддерживающих данный тип группировки.
  В поле устройство задаётся имя устройства. Например "CEIV-10AVG1-R304-CORE-5525X-FW1_1".
  В поле object-group задаётся имя object-group из устройства "CEIV-10AVG1-R304-CORE-5525X-FW1_". Например "SRV_WMWARE_IF".
  В поле "IP / Network" задаётся ip-адрес или сеть в формате "10.0.0.0/24". Если поле оставить пустым, то будут
        выведены все объекты данной object-group. Если заданный ip-адрес или сеть не найдена в object-group, то 
        первой строкой будет напечатан текст "Совпадений не найдено."
        
Утилита "PAP Viewer":
    Утилита находит наименование систем привязанным к ip-адресам.
    В поле "Api Token Netbox" необходимо вставить личный токен из профиля netbox.rt.ru
    В поле "Ввод" задаётся список ip-адресов и/или сетей в формате "10.0.0.0/24".
    Ограничение в итерации поиска утилиты установлено в 1 минуту. При его превышении, нужно уменьшить количество
        искомых объектов/сетей.
        ...
        def download():
            ...
            def worker():
                try:
                    ...
                    if 'ReadTimeout' in e:
                        add_log(f"❌ Ошибка: netbox.rt.ru не отвечает.")
                    else:
                    # if os.path.exists(CONF_DIR):
                    #     # make_writable(rem_dir)
                    #     shutil.rmtree(CONF_DIR)
